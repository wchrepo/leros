...
# import wandb
import mlflow
from utils import whiten, reduce_mean, reduce_sum, clamp, safe_cfg
...
class PPOTrainer:
    ...
    def __init__(self,
        ...
        if not args.nosave:
            ...
            if args.mode == 'eval':
                mlflow.set_experiment("Eval-rainier-tasks")
                mlflow.start_run(run_name=args.run_name)
                mlflow.log_params(safe_cfg(vars(args))) #之后再改吧
            else:
    ...
    def loss(self, results):
        ...
        new_values = value_forward['response/value'].to(device=self.policy_model.device)
    ...
    def train(self, step):
        ...
        with torch.no_grad(): # treat the values at beginning of step as ground-truth
            ...
            results['response/value'] *= results['response/mask'].to(self.policy_model.device)  # TODO: I doubt if this line is necessary
        ...
        if not self.args.nosave and step % self.args.log_interval == 0:
            ...
            mlflow.log_metrics(stats,step)
            # for k, v in stats.items():
            #     self.writer.add_scalar(k, v, step)

    def valid(self, step):
        self.optimizer.zero_grad()
        if step == 0 and self.args.skip_first_valid:
            return
        ...
        results_table = "|step|i|task|question|knowledge|pred|answer_ix|correct|\n"\
            "|-|-|-|-|-|-|-|-|\n"
        count = 0
        for i, batch in enumerate(tqdm(self.eval_dataloader)):
            if self.args.max_valid_batches and count>=self.args.max_valid_batches:
                break
            count+=1
            with torch.no_grad():
                rollouts = self.policy_model.sample(
                    text=batch['question'],
                    # top_p=0.0,#-inf error, change to top_k
                    top_k=1
                )
            ...
            results_table+="|{}|{}|{}|{}|{}|{}|{}|{}|\n".format(step, i, batch['task'][0], batch['question'][0], knowledges[0], results['preds'][0], batch['answer_ix'][0], results['corrects'][0])
        ...
        if self.args.nosave:
            ...
            mlflow.log_metrics(stats,step)
            mlflow.log_text(results_table,f"valid_{step}.md")
    ...
    def eval(self, step,givenk=False): # step=-1 for baseline
        self.log.info(f'Evaluating [ppo_step {step}] ...')
        ...
        for i, batch in enumerate(tqdm(self.eval_dataloader)):
            with torch.no_grad():
                ...
                if givenk:
                    #knowledges (K,B) 这种结构的原因是get_reward_ensemble的风格。
                    knowledgess = [[knowledges[i] for knowledges in batch['knowledges']] for i in range(len(batch['knowledges'][0]))]
                elif step != -1:
    ...
    def set_reward_norm(self):
        ...
        for batch in tqdm(self.train_dataloader):
            count+=1
            if self.args.max_reward_norm_batches and count >= self.args.max_reward_norm_batches:
                break
            results = self.policy_model.sample(
                text=batch['question'],
                temperature=self.args.temperature,
