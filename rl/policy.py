...
from rl.t5 import T5ForConditionalGenerationAndTokenRegression
...
class Policy:
    ...
    def sample(self,
        ...
        tokenized = self.tokenizer.batch_encode_plus(
            text,
            return_tensors='pt', padding=True, truncation='longest_first', max_length=self.max_input_len)
    ...
    def forward_pass(self,
        ...
        response_input_ids = response_input_ids.to(device=self.device)
        response_mask = response_mask.to(device=self.device)
        outputs = self.model(
