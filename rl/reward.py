...
class Reward:
    ...
    def infer(self,
        ...
        for i in range(0, len(flattened_prompts), self.batch_size):
            ...
            tokenized_prompts = self.tokenizer(batch_prompts, return_tensors='pt', padding=True, truncation='longest_first', max_length=self.max_input_len).to(self.device)
            tokenized_choices = self.tokenizer(batch_choices, return_tensors='pt', padding=True, truncation='longest_first', max_length=self.max_input_len).to(self.device)
        ...
        for idx, sz in enumerate(num_ans):
            answer_logits[idx, :sz] = -all_losses[cur_arr_idx:cur_arr_idx+sz] #注意answer_logits是用交叉熵（loss）平均值计算的，所以没问题
            cur_arr_idx += sz
        ...
        if self.reward_shape == 0: # r = p(a*|q,k) - p(a*|q), same as using --prob_diff before
            ...
            if knowledges[0] is not None:
                knowless_results = self.get_reward(
                    questions, None, choicess, answer_ixs, override_gain, override_bias)
                rewards -= torch.tensor(knowless_results['rewards/raw'], device=self.device)
        elif self.reward_shape == 14: # r = ck*sign(rq)*tanh(s(k))+ (1-ck)rq = { tanh[s(a*|q,k) - max s(a'|q,k)] - tanh[s(a*|q) - max s(a'|q)] } / 2
            distractor_idxs = answer_logits.argsort(dim=1, descending=True)
            take_second = (distractor_idxs[:, :1] == gt_idxs).long()
            ...
            if knowledges[0] is not None:
                knowless_results = self.get_reward(
                    questions, None, choicess, answer_ixs, override_gain, override_bias)
                rewards -= torch.tensor(knowless_results['rewards/raw'], device=self.device)
                knowledges_scores = self.kq_model(knowledges)
                rewards = self.kq_coef*torch.sign(rewards)*knowledges_scores + (1-self.kq_coef)*rewards
    ...
    def get_reward_ensemble(self,
                            ...
                            override_gain = None,
                            override_bias = None,
                            include_no_knowledge=True,
                           ) -> Tuple[List[float], float, int, int]:
        answer_logitss = []
        answer_probss = []
        if include_no_knowledge:
            knowless_results = self.get_reward(questions, None, choicess, answer_ixs, override_gain, override_bias,skip_reward=True)
